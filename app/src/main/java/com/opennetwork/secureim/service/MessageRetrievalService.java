...
import com.opennetwork.secureim.gcm.GcmBroadcastReceiver;
import com.opennetwork.secureim.jobs.PushContentReceiveJob;
...
public class MessageRetrievalService extends Service implements InjectableType, RequirementListener {
  ...
  public void onDestroy() {
    ...
    sendBroadcast(new Intent("com.opennetwork.securesms.RESTART"));
  }
  ...
  private void setForegroundIfNecessary() {
    if (TextSecurePreferences.isGcmDisabled(this)) {
      ...
      builder.setContentTitle(getString(R.string.MessageRetrievalService_opennetwork));
      builder.setContentText(getString(R.string.MessageRetrievalService_background_connection_enabled));
      builder.setPriority(NotificationCompat.PRIORITY_MIN);
      builder.setWhen(0);
      builder.setSmallIcon(R.drawable.ic_opennetwork_grey_24dp);
  ...
  private void shutdown(OpenNetworkServiceMessagePipe pipe) {
    try {
      pipe.shutdown();
    } catch (Throwable t) {
      Log.w(TAG, t);
  ...
  public static @Nullable OpenNetworkServiceMessagePipe getPipe() {
    return pipe;
  }
  ...
  private class MessageRetrievalThread extends Thread implements Thread.UncaughtExceptionHandler {
    ...
    public void run() {
      while (!stopThread.get()) {
        ...
        OpenNetworkServiceMessagePipe localPipe = pipe;
