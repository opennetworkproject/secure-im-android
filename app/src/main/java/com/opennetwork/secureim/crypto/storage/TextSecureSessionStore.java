...
import com.opennetwork.libim.protocol.CiphertextMessage;
import com.opennetwork.libim.state.SessionRecord;
import com.opennetwork.libim.state.SessionState;
...
import static com.opennetwork.libim.state.StorageProtos.SessionStructure;
...
public class TextSecureSessionStore implements SessionStore {
  ...
  @Override
  public SessionRecord loadSession(@NonNull OpenNetworkProtocolAddress address) {
  ...
  @Override
  public void storeSession(@NonNull OpenNetworkProtocolAddress address, @NonNull SessionRecord record) {
    synchronized (FILE_LOCK) {
      try {
  ...
  @Override
  public boolean containsSession(OpenNetworkProtocolAddress address) {
  ...
  @Override
  public void deleteSession(OpenNetworkProtocolAddress address) {
  ...
  public void deleteAllSessions(String name) {
    ...
    deleteSession(new OpenNetworkProtocolAddress(name, OpenNetworkServiceAddress.DEFAULT_DEVICE_ID));

    for (int device : devices) {
      deleteSession(new OpenNetworkProtocolAddress(name, device));
    }
  }
  ...
  public void migrateSessions() {
    synchronized (FILE_LOCK) {
      ...
      for (File session : directory.listFiles()) {
        if (session.isFile()) {
          OpenNetworkProtocolAddress address = getAddressName(session);
  ...
  private File getSessionFile(OpenNetworkProtocolAddress address) {
  ...
  private String getSessionName(OpenNetworkProtocolAddress address) {
    int deviceId = address.getDeviceId();
    return address.getName() + (deviceId == OpenNetworkServiceAddress.DEFAULT_DEVICE_ID ? "" : "." + deviceId);
  }

  private @Nullable OpenNetworkProtocolAddress getAddressName(File sessionFile) {
    try {
      ...
      else                  deviceId = OpenNetworkServiceAddress.DEFAULT_DEVICE_ID;

      return new OpenNetworkProtocolAddress(parts[0], deviceId);
    } catch (NumberFormatException e) {
      Log.w(TAG, e);
      return null;
    }
